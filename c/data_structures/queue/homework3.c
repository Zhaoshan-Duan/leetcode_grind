...
void enqueue(queue_t* q_p, int d) {
  ...
  if (q_p != NULL) {
    if (isEmpty(q_p)) {
      // queue is empty so point both head and tail to the new node
      q_p->head_p = n_p;
      q_p->tail_p = n_p;
    } else {
      // queue is not empty

      // point right of new node to tail
      n_p->right_p = q_p->tail_p;
      // tail node's left to new node
      q_p->tail_p->left_p = n_p;
      // update new node as the tail node
      q_p->tail_p = n_p;
